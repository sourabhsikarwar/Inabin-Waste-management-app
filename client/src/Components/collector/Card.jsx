  ...
  return (
    <div className='w-full p-4 flex justify-between h-32 rounded-lg shadow-md'>
      ...
      <div className='flex flex-col items-center justify-between'>
        ...
        <button className='bg-green hover:bg-darkGreen py-1 px-4 text-white rounded-lg' onClick={() => window.location.href = `http://localhost:5173/collector/request/${props.data._id}`}>{props.data.status === "started" ?"View" : "Collect" }</button>
      </div>
    </div>
  )
}
